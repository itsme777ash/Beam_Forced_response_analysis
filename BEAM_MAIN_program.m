E = 210e9; q = 0; P = 0; M = 0; density = 7850;b = 0.05; h = 0.01; I =b*h*h*h/12;
...
for i = 1:1:40
    syms y(t) omega
    eqn = diff(y,t,2) == -Mod_K(i,i)*y + Mod_F(i,1)*cos(omega*t);
    Dy = diff(y,t);
    cond = [y(0)==0, Dy(0)==0];
...
temp = zeros(length(1:1:50),1);
y = Final_ans(39);
Amp = zeros(length(5:1:100),1);
for f = 5:1:100
y_2 = subs(y,omega,2*pi*f);
    for tim = 1:1:50 
...
%plot(linspace(0.01,2,200),temp(:,1));
